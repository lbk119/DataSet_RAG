...
def node_entry(state: ImportGraphState) -> ImportGraphState:
    ...
    if not local_file_path:
        logger.error(f"【{func_name}】核心参数缺失：工作流状态中未配置 local_file_path，文件路径为空")
        return state
    # 2. 根据文件后缀判断类型，设置对应解析开关
    if local_file_path.endswith(".pdf"):
        logger.info(f"【{func_name}】文件类型校验通过：{local_file_path} → PDF格式，开启PDF解析流程")
        state["is_pdf_read_enabled"] = True
        state["pdf_path"] = local_file_path
    elif local_file_path.endswith(".md"):
        logger.info(f"【{func_name}】文件类型校验通过：{local_file_path} → MD格式，开启MD解析流程")
        state["is_md_read_enabled"] = True
        state["md_path"] = local_file_path
    else:
        logger.warning(f"【{func_name}】文件类型校验失败：{local_file_path} → 不支持的格式，仅支持.pdf/.md")
