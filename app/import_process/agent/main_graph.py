...
# 兼容 `python app/import_process/agent/main_graph.py` 直接启动。
...
workflow.add_edge(START, "node_entry") # 定义流程起点到入口节点的连接
def router_after_entry(state: ImportGraphState) -> str:
    """
    node_entry 之后的路由函数，根据输入文件类型决定流程走向。
    如果输入是 PDF，走 PDF 转 MD 的流程；如果输入是 MD，直接进入图片处理。
    """
    if state.get("is_pdf_read_enabled"):
        ...
        logger.error("未识别的文件类型，无法路由！")
        return END
    
workflow.add_conditional_edges("node_entry", router_after_entry,{
    "node_pdf_to_md":"node_pdf_to_md",
    "node_md_img":"node_md_img",
    END: END
}) 
workflow.add_edge("node_pdf_to_md", "node_md_img") # PDF转MD完成后，进入MD图片处理
workflow.add_edge("node_md_img", "node_document_split") # 图片处理完成后，进入文档分块
workflow.add_edge("node_document_split", "node_item_name_recognition") # 分块完成后，进入项目名识别
workflow.add_edge("node_item_name_recognition", "node_bge_embedding") # 项目名识别完成后，进入BGE向量化
workflow.add_edge("node_bge_embedding", "node_import_milvus") # 向量化完成后，进入Milvus导入
workflow.add_edge("node_import_milvus", END) # Milvus导入完成，流程结束
...
if __name__ == "__main__":
    ...
    # 4. 构造测试状态（贴合实际业务入参，开启PDF解析开关）
        ...
        try:
            ...
            final_state = None
            for step in import_graph.stream(test_state, stream_mode="values"):
            # 打印当前执行完成的节点（流式输出更直观）
            ...
            if final_state:
                ...
                logger.info(f"🗄 所有切片是否完成Milvus入库（含chunk_id）：{'是' if has_chunk_id else '否'}")
